...
export function NewSessionForm({ initialData, sessionId }: NewSessionFormProps = {}) {
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-6 bg-container rounded-3xl p-6 border border-foreground/5 shadow-xl mx-5 -mt-8 relative z-10">
      ...
      <div className="pt-4 space-y-3">
        ...
        <Link href={sessionId ? `/panel/clase/${sessionId}` : '/panel'}>
          <button 
            type="button" 
            className="w-full bg-white text-black font-bold h-14 rounded-xl flex justify-center items-center hover:bg-gray-100 transition-colors"
          >
            Cancelar
          </button>
        </Link>
