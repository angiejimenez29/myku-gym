...
export function RefundItem({ refund, hideSessionInfo = false }: { refund: RefundWithDetails, hideSessionInfo?: boolean }) {
    ...
    <>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              {reservation?.client_phone && (
                <button 
                  onClick={handleCopy}
                  className="text-xs bg-foreground/5 hover:bg-foreground/10 px-2 py-0.5 rounded-md flex items-center gap-1 transition-colors"
                >
                  {copied ? <span className="text-green-500">Copiado ✓</span> : <span>Copiar</span>}
                </button>
              )}
            </div>
          </div>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-background rounded-3xl p-6 w-full max-w-sm border border-foreground/10 shadow-2xl">
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-3 rounded-xl font-bold bg-foreground/5 text-foreground hover:bg-foreground/10 transition-colors"
              >
                Cancelar
              </button>
              <button 
